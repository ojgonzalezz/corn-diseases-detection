...
def analyze_dataset_distribution():
    ...
    print("=" * 60)
    print("1. DISTRIBUCION DE CLASES")
    print("=" * 60)
    ...
    print(f"\nTotal de imagenes: {df['Cantidad'].sum()}")
...
def analyze_image_properties():
    ...
    print("\n" + "=" * 60)
    print("2. PROPIEDADES DE LAS IMAGENES")
    print("=" * 60)
        ...
        for file in files:
            try:
                ...
                properties.append({
                    ...
                    'tamano_kb': os.path.getsize(file) / 1024
                })
            except:
                pass
    ...
    for cls in CLASSES:
        ...
        print(f"  Tamano: {cls_data['tamano_kb'].mean():.1f} +/- {cls_data['tamano_kb'].std():.1f} KB")
    ...
    for _, row in resolutions.iterrows():
        print(f"  {int(row['ancho'])}x{int(row['alto'])}: {row['count']} imagenes")
...
def analyze_pixel_statistics():
    ...
    print("\n" + "=" * 60)
    print("3. ESTADISTICAS DE PIXELES")
    print("=" * 60)
...
def create_visualizations(dist_df, prop_df, pixel_df):
    ...
    ax1.set_title('Distribucion de Clases (Procesado)', fontsize=12, fontweight='bold')
    ax1.set_xticklabels(ax1.get_xticklabels(), rotation=45, ha='right')

    # 2. Proporción de clases
    ax2 = plt.subplot(3, 4, 2)
    ax2.pie(dist_df['Cantidad'], labels=dist_df['Clase'], autopct='%1.1f%%', startangle=90)
    ax2.set_title('Proporcion de Clases', fontsize=12, fontweight='bold')
    ...
    ax3.set_title('Distribucion de Anchos', fontsize=12, fontweight='bold')
    ...
    ax4.set_title('Distribucion de Altos', fontsize=12, fontweight='bold')
    ...
    sns.violinplot(data=prop_df, x='clase', y='tamano_kb', palette='Set2', ax=ax6)
    ax6.set_title('Tamano de Archivo (KB)', fontsize=12, fontweight='bold')
    ...
    ax10.set_xlabel('')
    ax10.axhline(y=120, color='red', linestyle='--', alpha=0.5, label='Target')
    ...
    ax12.set_title('Correlacion Canales RGB', fontsize=12, fontweight='bold')

    plt.tight_layout()
    plt.savefig('eda_processed_data.png', dpi=150, bbox_inches='tight')
    print("\nVisualizaciones guardadas en: eda_processed_data.png")

def show_sample_images():
    """Mostrar muestras de cada clase"""
    print("\n" + "=" * 60)
    print("5. MUESTRAS DE IMAGENES")
    print("=" * 60)
    ...
    plt.suptitle('Muestras de Imagenes por Clase (Procesadas)', fontsize=16, fontweight='bold')
    plt.tight_layout()
    plt.savefig('samples_processed_data.png', dpi=150, bbox_inches='tight')
    print("\nMuestras guardadas en: samples_processed_data.png")
...
def generate_summary_report(dist_df, prop_df, pixel_df):
    ...
    report = []
    report.append("RESUMEN EJECUTIVO - EDA DATASET PROCESADO")
    report.append("=" * 60)
    report.append(f"\nTotal de imagenes: {dist_df['Cantidad'].sum()}")
    report.append(f"Numero de clases: {len(CLASSES)}")
    report.append(f"Clases: {', '.join(CLASSES)}")

    report.append(f"\n--- Balance de clases ---")
    report.append(f"Clase mas frecuente: {dist_df.loc[dist_df['Cantidad'].idxmax(), 'Clase']} ({dist_df['Cantidad'].max()} imgs)")
    ...
    report.append(f"Tamano promedio: {prop_df['tamano_kb'].mean():.1f} KB")

    # Verificar uniformidad
    unique_dims = prop_df[['ancho', 'alto']].drop_duplicates()
    if len(unique_dims) == 1:
        report.append(f"Uniformidad de dimensiones: PERFECTA (todas {int(unique_dims.iloc[0]['ancho'])}x{int(unique_dims.iloc[0]['alto'])})")
    else:
        report.append(f"Uniformidad de dimensiones: {len(unique_dims)} resoluciones diferentes")
    ...
    # Verificar normalización de brillo
    target_brightness = 120
    brightness_deviation = abs(pixel_df['brillo'].mean() - target_brightness)
    report.append(f"\nDesviacion del brillo target (120): {brightness_deviation:.2f}")

    report.append(f"\n--- Estadisticas por clase ---")
    for cls in CLASSES:
        cls_data = pixel_df[pixel_df['clase'] == cls]
        report.append(f"\n{cls}:")
        report.append(f"  Brillo: {cls_data['brillo'].mean():.2f} +/- {cls_data['brillo'].std():.2f}")
        report.append(f"  Contraste: {cls_data['contraste'].mean():.2f} +/- {cls_data['contraste'].std():.2f}")
        report.append(f"  RGB: R={cls_data['media_r'].mean():.2f}, G={cls_data['media_g'].mean():.2f}, B={cls_data['media_b'].mean():.2f}")

    report.append(f"\n--- Evaluacion de calidad ---")

    # Verificaciones
    balance_ok = (dist_df['Cantidad'].min() / dist_df['Cantidad'].max()) == 1.0
    dims_ok = len(unique_dims) == 1
    brightness_ok = brightness_deviation < 10

    report.append(f"[{'OK' if balance_ok else 'WARN'}] Balance de clases: {(dist_df['Cantidad'].min() / dist_df['Cantidad'].max() * 100):.2f}%")
    report.append(f"[{'OK' if dims_ok else 'WARN'}] Uniformidad de dimensiones: {'PERFECTA' if dims_ok else 'VARIADA'}")
    report.append(f"[{'OK' if brightness_ok else 'WARN'}] Normalizacion de brillo: desviacion={brightness_deviation:.2f}")

    if balance_ok and dims_ok:
        report.append("\n[LISTO] Dataset procesado listo para entrenamiento")
    else:
        report.append("\n[ATENCION] Revisar advertencias antes de entrenar")

    report_text = '\n'.join(report)
    print(report_text)

    with open('eda_processed_report.txt', 'w', encoding='utf-8') as f:
        f.write(report_text)
    print("\nReporte guardado en: eda_processed_report.txt")

def compare_with_original():
    """Comparar con dataset original"""
    print("\n" + "=" * 60)
    print("7. COMPARACION CON DATASET ORIGINAL")
    print("=" * 60)

    # Dataset procesado
    processed_counts = {}
    for cls in CLASSES:
        processed_counts[cls] = len(list((DATA_DIR / cls).glob('*')))
    ...
    if original_dir.exists():
        for cls in CLASSES:
            original_counts[cls] = len(list((original_dir / cls).glob('*')))

        comparison = []
        print("\n--- Comparacion de cantidades ---")
        print(f"{'Clase':<20} {'Original':>10} {'Procesado':>10} {'Diferencia':>12}")
        print("-" * 60)
        for cls in CLASSES:
            orig = original_counts.get(cls, 0)
            proc = processed_counts[cls]
            diff = proc - orig
            print(f"{cls:<20} {orig:>10} {proc:>10} {diff:>12}")
            comparison.append({'clase': cls, 'original': orig, 'procesado': proc, 'diferencia': diff})

        total_orig = sum(original_counts.values())
        total_proc = sum(processed_counts.values())
        print("-" * 60)
        print(f"{'TOTAL':<20} {total_orig:>10} {total_proc:>10} {total_proc - total_orig:>12}")

        # Balance comparison
        orig_balance = (min(original_counts.values()) / max(original_counts.values()) * 100)
        proc_balance = (min(processed_counts.values()) / max(processed_counts.values()) * 100)

        print(f"\n--- Mejora en balance ---")
        print(f"Balance original: {orig_balance:.2f}%")
        print(f"Balance procesado: {proc_balance:.2f}%")
        print(f"Mejora: +{proc_balance - orig_balance:.2f} puntos porcentuales")
    else:
        print("\nDataset original no encontrado para comparacion")

def main():
    """Función principal"""
    print("\n" + "=" * 60)
    print("EDA COMPLETO - DATASET PROCESADO")
    print("=" * 60)
    ...
    print("=" * 60)
    print("\nArchivos generados:")
    print("  - eda_processed_data.png (visualizaciones)")
    print("  - samples_processed_data.png (muestras)")
    print("  - eda_processed_report.txt (reporte resumen)")
    print("\n")
