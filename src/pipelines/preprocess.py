...
def data_split(datasets: Dict[str, Any], environment_variables: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        split_ratios = tuple(ast.literal_eval(env_vars.get("SPLIT_RATIOS", "()")))
    except:
        split_ratios = (0.7, 0.15, 0.15)
        print(f"Split ratios: se ha usado el split ratio tuple de emergencia!, split_ratios = {split_ratios}")
            
    if not categories:
        raise ValueError("No se pudieron determinar los nombres de las clases (categories).")
    if not split_ratios:
        raise ValueError("No se pudieron determinar split ratios ni cargar el split ratio tuple de emergencia.")

    raw_keys_to_iterate = [key for key in datasets.keys() if key.startswith("data_")]


    # 1. Inicialización del dataset por categoria, contando que un oversampling, subsampling o ninguno haya sido efectuado previamente
    all_images_categories = {
        cat: [] for cat in categories
    }

    for dataset_key in raw_keys_to_iterate:
        for cat in categories:
            all_images_categories[cat].extend(
                datasets[dataset_key]["images"][cat]
            )
    
    # 2. Determinar el tamaño de cada categoría
    total_lenghts = {cat: len(all_images_categories[cat]) for cat in categories}

    # Tamaño de los tres splits con base a los splits ratios y a las imagenes por categoría
    total_size = sum(list(total_lenghts.values()))
    train_set_size = int(total_size * split_ratios[0])
    val_set_size = int(total_size * split_ratios[1])
    test_set_size = int(total_size * split_ratios[2])
...
def downsample_dataset(datasets: Dict[str, Any], environment_variables: Dict[str, Any]) -> Dict[str, Any]:
    """
...
def split_and_balance_dataset(split_ratios: tuple = (0.7, 0.15, 0.15), balanced: str = "downsample"):
    ...
    if balanced == "downsample":
        ...
        print(f"\n⚖️  Modo balanceado: Todas las clases se reducirán a por submuestreo.")
        downsampled_datasets = downsample_dataset(datasets)
